 ...
 */
?>
<form id="posts-filter" action="<?php echo EIRD_GetCallBackURL(); ?>" method="post">
...
<div class='tablenav-pages one-page'><span class="displaying-num">مبلغ کل حمایت شده :<?php echo get_option("EIRD_TotalAmount"); ?> تومان</span>
</div>
</div>
...
?>
		...
		<tr id="post-109" style="<?php if ($row->Status == 'OK') {
    echo 'background-color: #cfc';
}
?>" class="post-109 type-post status-draft format-standard hentry category-news alternate iedit author-self" valign="top">
		...
		<td class="author column-author"><?php echo $row->AmountRial; ?></td>
...
for ($i = 1; $i <= $PageNumInt; $i++) {
    if ($i == $currentPage) {
        echo '<a href="admin.php?page=EIRD_Hamian&pageid=' . $i . '"  class="first-page disabled">' . $i . '</a>';
    } else {
        echo '<a href="admin.php?page=EIRD_Hamian&pageid=' . $i . '"  class="first-page">' . $i . '</a>';
    }
